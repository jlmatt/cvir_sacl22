...
mort_dat <- mort_dat %>%
  mutate(recovery = if_else(death_date >= "2022-10-21" & treatment == "L","y",
                    ...
                    if_else(death_date >= "2022-10-21" & treatment == "C","y","n"))))
```
...
```{r}
saveRDS(control_models_parameters, file = here::here("data","derived","control_low_models_parameters"))
```
...
c2_r_effects<-as_tibble(ranef(c2)) %>%
  rownames_to_column() %>%
...
```{r}
saveRDS(mort, file = here::here("data","derived","06_best_model_control_low_mort"))

saveRDS(total, file = here::here("data","derived","06_best_model_control_low_total"))

saveRDS(c2_table, file = here::here("data","derived","06_best_model_control_low_fixed_effects"))

saveRDS(c2_r_effects, file = here::here("data","derived","06_best_model_control_low_random_effects"))
```
...
```{r}
t<-residuals(c2_fixed,type="deviance")
