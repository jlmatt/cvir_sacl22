...
```{r}
suc <-readRDS(file = here::here("data","derived","control_low_models_parameters")) %>%
...
data <- bind_rows(suc, failed_models) %>%
  mutate(Model = c("m2", "m1","m4","m3","m5","m6","m7"))
```
...
```{r}
gtsave(t1, here::here("out","tbl","TableA4_low_control_models_parameters.png"))
```
